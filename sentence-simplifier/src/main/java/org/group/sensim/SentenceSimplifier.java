package org.group.sensim;// org.group.sensim.Question Generation via Overgenerating Transformations and Ranking
 ...
 * M.  Heilman and N. A. Smith. 2010. Good org.group.sensim.Question! Statistical Ranking for org.group.sensim.Question Generation. In Proc. of NAACL/HLT.
 *
 * The following paper discusses the methods implemented here, and provides some experimental evaluation results:
 * M. Heilman and N. A. Smith. 2010. Extracting Simplied Statements for Factual org.group.sensim.Question Generation. In Proc. of the 3rd Workshop on org.group.sensim.Question Generation.
 *
 * When creating the output, the system makes various calls to org.group.sensim.Question.setFeatureValue().
...
public class SentenceSimplifier {
     ...
     * and returns a list of org.group.sensim.Question objects, which help
    ...
    /**
     * Method to add a new org.group.sensim.Question object q to a given set
    ...
    private void extractConjoinedPhrases(Collection<Question> extracted, Question input) {
        ...
        while(matcher.find()){
            ...
            //make a new org.group.sensim.Question object and add it
    ...
    private void extractAppositives(Collection<Question> extracted, Question input) {
        ...
        while(matcher.find()){
			...
			/*if(nountree.headPreTerminal(org.group.sensim.AnalysisUtilities.getInstance().getHeadFinder()).label().toString().equals("NNP")
					&& appositivetree.headPreTerminal(org.group.sensim.AnalysisUtilities.getInstance().getHeadFinder()).label().toString().equals("NNP"))
			{
				continue;
			}*/
    ...
    /**
     * If multiple ADJPs present in one sentence, they should be split into multiple.
     *
     * "The book is yellow and old." --> The book is yellow. The book is old.
     * "The book is very yellow and pretty old." --> The book is very yellow. The book is pretty old. The book is yellow. The book is old.
     * "The architectural styles reflect American, Spanish, Chinese, and Malay influences."
     * --> The architectural styles reflect American influences. The ... Spanish influences.
     *
     * @param extracted
     * @param input
     */
    private void extractADJPs(Collection<Question> extracted, Question input) {
        String tregexOpStr;
        TregexPattern matchPattern;
        TregexMatcher matcher;
        Tree newTree;
